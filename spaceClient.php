...
if(!isset($_SESSION['IdClient']))//a faire avec !isset
{
   header("location:AuthentificationClient.php");
}

$select = "select * from client where IdClient=" . $_SESSION['IdClient'];
...
<header class="header">
    <div class="logo"><span>Made In </span>Ngaye</div>
        ...
        <div class=" " style="text-align:left;" id="conteneurA">
            <h2><?php echo"Bienvenue  ".$row['Prenom'].' '.$row['Nom']?></h2>
        </div>
</header>
<br/>
<br/>
...
$access = "SELECT * FROM commande c where c.IdClient=". $_SESSION['IdClient']." order by IdCommande desc";
